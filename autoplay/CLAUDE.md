...
| `inference.py` | Threat-filtered `IncomingProjectile` (approaching + in-lane + impact window); `ClosingEnemy` (a `Grunt` heading into the union of its actor's rear-attack bands within `CLOSING_ENEMY_THREAT_TICKS`, `check_for_closing_enemies` -- never for `Boss`, out of scope; deliberately independent of `decide.py`, mirroring the existing no-cross-import convention between the two modules) -- the early-warning signal for a fast diagonal closer that would otherwise go undetected by the purely instantaneous-position band checks in `decide.py`; non-playable `Actors` are filtered by not being `Myself`/`Partner`/`Enemy` |
