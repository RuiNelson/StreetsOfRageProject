...
`HELD_BY_ENEMY`, and the counter's C then B throws the holder). Not done
yet: keeping the AI from walking into the partner in the first place, so a
brief grab and release can still happen.
