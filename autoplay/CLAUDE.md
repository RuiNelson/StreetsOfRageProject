...
The suplex chain is what carries the fight now (2-3 suplexes plus knees per
fight, against 0 before). `tools/antonio_diag.py` is the per-tick
diagnostic that found all of this: it runs the real pipeline and logs every
candidate verb with its emergency next to Antonio's own bytes.

**Still open, and measured:** every remaining hit he lands arrives while the
actor is **airborne in the ROM's held-weapon jump family `$3C`-`$42`** --
the armed hop, which is not the `$16` jump kick the tactic is built on and
carries no kick at all. Refusing the weapon detour outright whenever a live
Antonio is on screen (rather than only inside his kick window) was tried for
exactly this and **measured as no better**: three fights gave 20 / 40 / 160
damage taken with one death, against 20 / 40 / 40 and none for the shipped
gate, so it was reverted rather than kept on the strength of the reasoning.
...
first later-boss with dedicated tactics (`reach.antonio_will_kick` /
`DodgeAntonioKick` / `HitAntonioBoomerang` / `grab_reasons`' `ANTONIO_ON_PUNISH`). The
fight is jump-kick → grab → suplex: `_could_melee_strike` refuses every
grounded B on Antonio (the first punch is the same `$16EAE` zero-velocity
...
lands in grab range facing him -- a directed hop from there flew past,
landed facing away, and a live fight recorded 374 `JumpAttack` with
0 `GrabEnemy`; later-boss primary `$03`/`$04` decode as `RECOVERY` so
