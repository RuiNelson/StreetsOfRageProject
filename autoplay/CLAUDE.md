...
| `recovery_decisions.py` | `TechRecover` — fires while `PlayableCharacter.throw_tech_ready` (armed `+$45` on a techable action `$5C`/`$72`/`$88`); `should_tech_recover` bypasses the generic `_blocked` gate like `should_counter_grab`, since the actor is airborne/hurt for the whole window |
| `decide.py` | `_live_enemies` excludes any enemy outside the level's playable Y lane (`lane_y_max_for_level`) -- e.g. stage 1's scripted "behind a door" placeholder, a real tracked Enemy the player can never reach -- so it can't be targeted or block stage advance; `should_*` generators; `should_walk_to_near_enemy` prefers an on-screen chase but falls back to the nearest live enemy ahead in the stage's own scroll direction when nothing is on-screen (never one behind, per `_ahead_in_stage_direction`) -- otherwise a live off-screen enemy holds back stage advance while nothing ever moves the camera toward it; stage advance gated on *every* live enemy (on-screen or not), not just on-screen ones -- except an off-screen enemy already at exactly 0 health (`_advance_blocking_enemies`), which nothing here will ever chase down to finish off; hold always acts |
| `priority.py` | Emergency is computed per concrete `Decision` class from the `Information` tokens present in `Context` (never from the decision's type alone) — one `_emergency_*` function per class (or a shared function for the four melee-strike siblings, or a `_held_enemy_emergency` factory for the five hold moves), dispatched by a `type(decision) → function` table; module constants are named contributions, not static outcomes. Scores: counter-grab 100 (Myself held by enemy), tech-recover 90 (Myself throw_tech_ready), call-police 88 (Myself health below a lives-aware threshold: POLICE_HEALTH_PERCENT_THRESHOLD, raised to POLICE_HEALTH_PERCENT_THRESHOLD_LAST_LIFE on the last life), rear 60/55 (target dangerous / not), supplex 68 / throw-held 70 / flip 66 / knee 64 / release 50 (target `Enemy` actually `GRABBED`), knife/pepper 25 each (target beyond melee, within throw range), jump 28/18 and melee-strike (punch/swing/stab/spray, shared formula) 60/20 (target punishable / not), breakable 16/14, weapon 8 (floor `Weapon` outranks the held one), walk-to-near-enemy 14, stage-advance 12 (no *blocking* `Enemy` anywhere -- `_advance_blocking_enemies` excludes an off-screen straggler stuck at 0 health), pickup tiers 50/15/12/9/3 — the max wins, with the `priority` field breaking ties; hold throws outrank knees |
