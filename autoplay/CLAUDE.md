...
| `enemy.py` | `Enemy` (a `Character`; adds `type_id`, `targets_player`) + per-type/boss subclasses: `Garcia`/`Signal`/`HakuRo`/`Nora`/`Jack`, `Boss` → direct subclasses `Abadede`/`MrX`/`Souther`/`Antonio`/`Bongo`/`Onihime` (tactical fields with defaults on `Boss`); `enemy_class_for_type` |
