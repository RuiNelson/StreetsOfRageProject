...
| `world_map.py` | Camera + actors → map entities (incl. hunt targets); `MapEntity.enemy_vel_x`/`enemy_vel_y` carry ordinary-enemy velocity (+$1C/+$20), read only in the `kind=="enemy"` branch -- distinct fields/offsets from the boss-only `vel_x`/`vel_z` (+$20/+$24) already on the same dataclass, left untouched |
...
| `inference.py` | Threat-filtered `IncomingProjectile` (approaching + in-lane + impact window); `ClosingEnemy` (a `Grunt` heading into its actor's *side-specific* rear-attack band -- behind vs front picked via the same facing test as `decide._enemy_behind_actor`, never their union, since Axel/Blaze have zero forward reach and a front-side union previously let a front-approaching enemy wrongly fire `RearAttack` for them -- within `CLOSING_ENEMY_THREAT_TICKS`, `check_for_closing_enemies`; never for `Boss`, out of scope; deliberately independent of `decide.py`, mirroring the existing no-cross-import convention between the two modules) -- the early-warning signal for a fast diagonal closer that would otherwise go undetected by the purely instantaneous-position band checks in `decide.py`; non-playable `Actors` are filtered by not being `Myself`/`Partner`/`Enemy` |
