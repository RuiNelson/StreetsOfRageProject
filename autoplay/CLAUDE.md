...
| `attack_decisions.py` | `Punch`, `SmashBreakable`, hold moves (`AttackHeldEnemy`/`ThrowHeldEnemy`/`FlipHold`/`Supplex`/`ReleaseGrab`), `JumpAttack` (horizontal only), `RearAttack`, `CounterGrab`; `MeleeAttacks` groups weaponless close combat (`Punch`/`JumpAttack`/`RearAttack`/`GrabMechanics`); `GrabMechanics` groups all grab/anti-grab moves |
