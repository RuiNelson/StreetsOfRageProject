...
class Projectile(Observed):
    ...
    only once thrown, so ``check_for_incoming_projectiles`` needs it to
    tell the two apart.
    """
...
class IncomingProjectile(Inferred):
    ...
    and, for Jack's axe/torch specifically, only once he has actually let
    go of it (``Jack.has_projectile`` false), not while it is still
    spinning through his juggle.
    """
