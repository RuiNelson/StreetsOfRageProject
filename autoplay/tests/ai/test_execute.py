...
class SoutherPocketApproachTests(unittest.TestCase):
    ...
    def test_the_approach_lines_up_on_his_lane(self) -> None:
        # User: "eu quero que o Y do inimigo e do jogador se alinhem". From
        # either side of him, and at the top of the band as well as lower
        # down, the aim is his own lane.
        for souther_y in (80, 10):
            souther = self._souther(world_x=200, world_y=souther_y)
            for actor_y in (100, 90, 70, 20):
                with self.subTest(souther_y=souther_y, actor_y=actor_y):
                    actor = _myself(world_x=100, world_y=actor_y)
...
class DodgeSoutherSlashExecuteTests(unittest.TestCase):
    ...
    def test_side_is_picked_from_southers_own_lane(self) -> None:
        ...
        # 14px below him: the deep edge (30px away) is now the nearer one.
        below = self._run(_myself(world_x=120, world_y=74), souther_y=60)
        self.assertTrue(below.hold_buttons.call_args.kwargs["player1"] & DOWN)

    def test_an_aligned_actor_leaves_by_the_nearest_edge(self) -> None:
        # The measured failure: aligned on him, an actor two px below his lane
        # read the bare sign of dy and took the claw's deep side -- 44px,
        # which it mostly never finished (48 of 78 hits landed below him,
        # after up to twenty ticks of dodging). From inside the claw the
        # nearest edge wins, and for an aligned actor that is the shallow one.
        client = self._run(_myself(world_x=120, world_y=62), souther_y=60)
        held = client.hold_buttons.call_args.kwargs["player1"]
        self.assertTrue(held & UP, f"expected the shallow edge, got {held:#06x}")
    ...
    def test_with_no_room_above_it_charges_the_pocket_instead(self) -> None:
        # He stands in the band's top rows: the shallow exit does not exist and
        # the deep one is the claw's full width. Forward is the short way, and
        # no hit has been recorded inside the pocket in any batch.
        #
        # Lane 10, not the band floor itself: with the actor at the floor
        # (lane 8, LANE_EDGE_MARGIN above LANE_Y_MIN) its 16px body pokes out
        # of the plannable world and the route's first vector is a 4px step
        # down to bring it back -- a property of nav.plan_route at the band's
        # edge, not of this dodge, whose target is the pocket on the same
        # lane from any starting lane.
        client = MagicMock()
        gamepad = VirtualGamepad(SharedGamepadState(client), player_index=1)
        actor = _myself(world_x=120, world_y=10)
        ...
        held = client.hold_buttons.call_args.kwargs["player1"]
        self.assertTrue(held & RIGHT, f"expected a charge into the pocket, got {held:#06x}")
        self.assertFalse(held & DOWN)
...
class SoutherLaneOffsetIsDroppedWhenTheGateCannotFireTests(unittest.TestCase):
    ...
    def test_his_lane_at_any_distance_and_tactical(self) -> None:
        for dx in (40, 90, 160):
