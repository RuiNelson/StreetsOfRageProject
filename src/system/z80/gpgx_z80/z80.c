 ...
 *****************************************************************************/
/* Vendored from Genesis-Plus-GX/core/z80/z80.c (Juergen Buchmueller, freeware
 * non-commercial). The original includes GPGX's "shared.h" for build-wide
 * declarations; with VERBOSE==0 (below) every symbol it would have supplied
 * (logerror/cpu_getactivecpu) compiles out, so only the CPU-local header and
 * its osd_cpu.h types remain. The only other build-wide macro it relies on is
 * INLINE (from GPGX's macros.h, not vendored here since nothing else in this
 * file needs it). */
#ifndef INLINE
